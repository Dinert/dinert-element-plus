...
<script setup>
    ...
    const clickCb = `(scope: ScopeProps<T>, column: RewriteTableColumnCtx<T>, item: OperationsProps<T>, e?: any) => void`
    const headerListPropsTooltip = `HeaderListPropsTooltip| ((item: HeaderListProps) => HeaderListPropsTooltip | any)`
</script>
...
| show       | 是否显示          | <dinert-api-typing type="enmu" :details="clickCb"/>           |        | 一 |
| sort       | 操作的排序，数值越小列越靠前 前                                 | Number                                                               | 一     |
| messageBox | messageBox的配置项  | [messageBox](https://element-plus.org/en-US/component/message-box.html#api)   | 一     |
| popConfirm | popConfirm的配置项  | [popConfirm](https://element-plus.org/zh-CN/component/popconfirm#attributes)   | 一     |
